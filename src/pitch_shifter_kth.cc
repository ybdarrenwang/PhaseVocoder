...
void PitchShifter::PitchShifting(float **Spectrum, float PitchQuotient, int frame_shift, bool reset_ph)
{
    ...
    for(i=0;i<FFT_SIZE/2+1;i++)
    {
        if (i==subband[i])
        {
            ...
            phasor[i] = phasor[prev_subband[i]] + (i*(PitchQuotient-1))*(2.0*PI*frame_shift)/FFT_SIZE;
    ...
    for(i=0;i<FFT_SIZE/2+1;i++)	
    {
        PQResidual[i] = PQResidual[subband[i]];
        DelFreqBin[i] = DelFreqBin[subband[i]];
...
void PitchShifter::PitchShifting_KTH(float **Spectrum, float PitchQuotient, int frame_shift, bool reset_ph, float *window, float pitch)
{
    ...
    for (i=0; i<iteration; i++)
    {
        ...
        if (mark_phasor[max_bin] == false)
        {
            ...
            phasor[max_bin] = phasor[prev_subband[max_bin]] + (i*(PitchQuotient-1))*(2.0*PI*frame_shift)/FFT_SIZE;
        ...
        for(j=0; j<FFT_SIZE/2+1; j++)
        {
            if (j-max_bin+FFT_SIZE/2 >= 0 && j-max_bin+FFT_SIZE/2 < FFT_SIZE)
            {
                ...
                {
                    /*					SpecTemp[0][j+DelFreqBin] += max_amp*window[j-max_bin+FFT_SIZE/2]*cos(StudentPha[j]+phasor[max_bin]);
                                                        SpecTemp[1][j+DelFreqBin] += max_amp*window[j-max_bin+FFT_SIZE/2]*sin(StudentPha[j]+phasor[max_bin]);
                     */
                    // interpolation
                    SpecTemp[0][j+DelFreqBin] += (1-PQResidual)*max_amp*window[j-max_bin+FFT_SIZE/2]*cos(StudentPha[j]+phasor[max_bin]);
                    SpecTemp[0][j+DelFreqBin+1] += PQResidual*max_amp*window[j-max_bin+FFT_SIZE/2]*cos(StudentPha[j]+phasor[max_bin]);
                    SpecTemp[1][j+DelFreqBin] += (1-PQResidual)*max_amp*window[j-max_bin+FFT_SIZE/2]*sin(StudentPha[j]+phasor[max_bin]);
                    SpecTemp[1][j+DelFreqBin+1] += PQResidual*max_amp*window[j-max_bin+FFT_SIZE/2]*sin(StudentPha[j]+phasor[max_bin]);
                }
