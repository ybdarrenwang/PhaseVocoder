...
void TimeStretcher::Stretch(vector<Frame*>& input_spec, vector<Frame*>& output_spec, int &synthesis_frame_shift, bool reset_phase) {
    ...
    for (unsigned frame_idx=0; frame_idx<input_spec.size(); ++frame_idx) {
        ...
        if (frame_idx > 0)
            UpdatePhase(mag, input_spec[frame_idx-1]->getPhase(), input_spec[frame_idx]->getPhase(), ph);
